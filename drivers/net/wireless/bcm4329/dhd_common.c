...
extern int dhd_keep_alive_onoff(dhd_pub_t *dhd, int ka_on);
extern bool ap_fw_loaded;

int dhd_set_btc_params70( dhd_pub_t *dhd , int para )
{
	char buf_reg70va[8] = { 70, 00, 00, 00, 0x00, 0x00, 0x00, 0x00 };
	char iovbuf[WL_EVENTING_MASK_LEN + 12];
	
	DHD_ERROR(("Set btc_params 70 = %d\n" , para ));
	buf_reg70va[4] = para & 0xff;
	buf_reg70va[5] = (para >> 8) & 0xff;
	buf_reg70va[6] = (para >> 16) & 0xff;
	buf_reg70va[7] = (para >> 24) & 0xff;
	memset( iovbuf , 0 , sizeof(iovbuf) );

	bcm_mkiovar("btc_params", (char *)&buf_reg70va[0], sizeof(buf_reg70va), iovbuf, sizeof(iovbuf));
...
dhd_preinit_ioctls(dhd_pub_t *dhd)
{
	...
	*/
	...
	dhd_set_btc_params70( dhd , 50 );

	/* Enable HT WSEC restrict for 11n certification */
...
#if 1//defined(KEEP_ALIVE)
        /* Set Keep Alive : be sure to use FW with -keepalive */
        if (ap_fw_loaded == FALSE) {
                if ((ret = dhd_keep_alive_onoff(dhd, 1)) < 0)
                        DHD_ERROR(("%s set keeplive failed %d\n", \
                __FUNCTION__, ret));

                DHD_ERROR(("dhd_keep_alive_onoff result:%d\n", ret));
                }
